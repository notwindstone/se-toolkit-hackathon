...
) as Statement<never, [number, string, number | null, string | null]>;
...
export function getRecentByTarget(targetId: number, limit = 50): Array<HealthCheck> {
  return getRecentStmt.all(targetId, limit);
}

export function getUptimePercent(targetId: number): number {
